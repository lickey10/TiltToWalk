...
public class ControlGUIScript : MonoBehaviour {
    ...
    public bool DisplayPlayButton = false;
	...
	void OnGUI() 
	{
		...
		if (DisplaySlider && IsPlaying != true)
		{
		...
		if (DisplayPlayButton && GUI.Button(new Rect(GBS.Position.x + Offset.x + LabelOffSet.x, GBS.Position.y + Offset.y + LabelOffSet.y - 80, 100, 25),playText ))
		{
			if (IsPlaying == true)
			{
				IsPlaying = false;
