...
public class GUIBarScript : MonoBehaviour {
    ...
    public bool DisplayLeftOrUp = false;
    ...
    void OnGUI()
	{
        if (Value <= 0 && DisplayLeftOrUp)
        {
            tempValue = Value * -1;
        }
        else if (Value >= 0 && !DisplayLeftOrUp)
        {
            tempValue = Value;
        }
        else
        ...
        //UpdateBar is a very large function so i'm only executing it when i have to.
        if (Mathf.Round (CurrentValue * 100f) != Mathf.Round (tempValue * 100f)
            //&& ((DisplayLeft && Value <= 0) || (!DisplayLeft && Value >= 0))
			)
        {
                UpdateBar();
        }
        ...
        if(!DisplayHorizontal)
            GUIUtility.RotateAroundPivot(-90.0f, new Vector2(transform.position.x, transform.position.y));

        if (DisplayLeftOrUp)
            GUIUtility.RotateAroundPivot(180.0f, new Vector2(transform.position.x, transform.position.y));

        //if you don't have a background texture i won't draw it
        if (Background != null)
		{
            //Rect newRect = new Rect(Position.x, Position.y, Background.width * ScaleSize, Background.height * ScaleSize);
            //newRect.ro
		...
		if (DisplayText)
		{
			...
			{
				Color MaxGradientColor = new Color(g.Evaluate(tempValue * 0.99f).r,g.Evaluate(tempValue * 0.99f).g,g.Evaluate(tempValue * 0.99f).b,1.0f);
			...
			if(TextString.Length == 0)
				tempTextString = ((int)(tempValue * 100)).ToString() + "%";
			else
				tempTextString = TextString;

            //capital Y of webdings is a heart
            Rect rectText = new Rect(Position.x + TextOffset.x, Position.y + TextOffset.y, ValueBar.width * ScaleSize, ValueBar.height * ScaleSize);

            if(!DisplayHorizontal)
                GUIUtility.RotateAroundPivot(-90.0f, new Vector2(rectText.x, rectText.y));

            GUI.Label(rectText, tempTextString,LabelStyle);
	...
	private void UpdateGradient () 
	{
		...
		FadeValue = ((Mathf.Sin ((tempValue) * 3.14f))/FadeFactor) ;
